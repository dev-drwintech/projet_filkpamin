...
@section('content')
    ...
    <section class="banner  w-100">
        <div class="col titltemovies">
            <h1 class="home__title mb-0 " style="color: white; ">Les Séries les mieux notés</h1>
        ...
        <div class="row">
            ...
            <div id="album-rotator">
                <div id="album-rotator-holder">
                    @if ($Series->isEmpty())
                        <p>Aucune meilleur série pour l'instant</p>
                    @else
                        @foreach ($Series as $Series)
                            <div class="card mx-4">
                                <div class="card__cover card_film">
                                    <img src="{{ asset('storage/' . $Series->photos) }}" alt="{{ $Series->title }}" />
                                    <!-- Vidéo qui sera affichée au survol -->
                                    <video class="card__video" muted preload="metadata">
                                        <source src="{{ asset('storage/' . $Series->demo) }}" type="video/mp4">
                                    </video>
                                    <a href="{{ route('frontend.videos.show', $Series->slug) }}" class="card__play">
                                ...
                                <div class="card__content">
                                    <div class="d-flex justify-content-between">
                                        <div>
                                            <span style="color: #3d3ddb">
                                                <i
                                                    class="fa-regular fa-clock mx-1"></i>{{ convertMinutesToHours($Series->runtime) }}
                                            </span>
                                        </div>
                                        <div>
                                            @if ($Series->views == 0)
                                    ...
                                    <p class="card__title">
                                        <a href="{{ route('frontend.videos.show', $Series->slug) }}"
                                            style="color: #fff;">{{ $Series->title }}</a>
                                    </p>
                                    <span class="card__category" style="color: #ffd80e !important;">
                                        {{ $Series->type }}
                                    </span>
                                </div>
                            </div>
            ...
            <div class="row">
                ...
                <div id="carousel10" class="carousel-container" data-aos="fade-up" data-aos-duration="2000">
                    <div id="carousel10-holder" class="carousel10-holder">
                        @php
                            $Toutseries = $data['Toutseries']->filter(function ($newseries) {
                                $genres = json_decode($newseries->genres);
                                return $newseries->status == 1 &&
                                    $newseries->type == 'Serie' &&
                                    is_array($genres) &&
                                    in_array('action', $genres);
                            });
                        @endphp

                        @if ($Toutseries->isEmpty())
                            <p>Aucune série d'action disponible pour l'instant</p>
                        @else
                            @foreach ($Toutseries as $newseries)
                                <div class="card mx-4">
                                    <div class="card__cover card_film">
                                        <img src="{{ asset('storage/' . $newseries->photos) }}"
                                            alt="{{ $newseries->title }}" />
                                        <!-- Vidéo qui sera affichée au survol -->
                                        <video class="card__video" muted preload="metadata">
                                            <source src="{{ asset('storage/' . $newseries->demo) }}" type="video/mp4">
                                        </video>
                                        <a href="{{ route('frontend.videos.show', $newseries->slug) }}" class="card__play">
                                            <i class="icon ion-ios-play" style="color:#ff6a00;"></i>
                                        </a>
                                    </div>
                                    <div class="card__content">
                                        <div class="d-flex justify-content-between">
                                            <div>
                                                <span style="color: #3d3ddb">
                                                    <i
                                                        class="fa-regular fa-clock mx-1"></i>{{ convertMinutesToHours($newseries->runtime) }}
                                                </span>
                                            </div>
                                            <div>
                                                @if ($newseries->views != 0)
                                                    <span style="color: #3d3ddb; font-weight: 600;">
                                                        {{ formatNumber($newseries->views) }}
                                                        <i class="fa-regular fa-eye mx-1"></i> vues
                                                    </span>
                                                @endif
                                            </div>
                                        ...
                                        <p class="card__title">
                                            <a href="{{ route('frontend.videos.show', $newseries->slug) }}"
                                                style="color: black;">{{ $newseries->title }}</a>
                                        </p>
                                        <span class="card__category">{{ $newseries->type }}</span>
                                    </div>
                                </div>
                            @endforeach
                        @endif
                        ...
                        @php
                            $Toutseries = $data['Toutseries']->filter(function ($newseries) {
                                ...
                                return $newseries->status == 1 &&
                                    $newseries->type == 'Serie' &&
                                    is_array($genres) &&
                                    in_array('aventure', $genres);
                            });
                        @endphp

                        @if ($Toutseries->isEmpty())
                            <p>Aucune série d'aventure disponible pour l'instant</p>
                        @else
                            @foreach ($Toutseries as $newseries)
                                <div class="card mx-4">
                                    <div class="card__cover card_film">
                        ...
                        @php
                            $Toutseries = $data['Toutseries']->filter(function ($newseries) {
                                ...
                                return $newseries->status == 1 &&
                                    $newseries->type == 'Serie' &&
                                    is_array($genres) &&
                                    in_array('science fiction', $genres);
                            });
                        @endphp

                        @if ($Toutseries->isEmpty())
                            <p>Aucune série de science fiction disponible pour l'instant</p>
                        @else
                            @foreach ($Toutseries as $newseries)
                                <div class="card mx-4">
                                    <div class="card__cover card_film">
                        ...
                        @php
                            $Toutseries = $data['Toutseries']->filter(function ($newseries) {
                                ...
                                return $newseries->status == 1 &&
                                    $newseries->type == 'Serie' &&
                                    is_array($genres) &&
                                    in_array('mystere', $genres);
                            });
                        @endphp

                        @if ($Toutseries->isEmpty())
                            <p>Aucune série mystérieux disponible pour l'instant</p>
                        @else
                            @foreach ($Toutseries as $newseries)
                                <div class="card mx-4">
                                    <div class="card__cover card_film">
                        ...
                        @php
                            $Toutseries = $data['Toutseries']->filter(function ($newseries) {
                                ...
                                return $newseries->status == 1 &&
                                    $newseries->type == 'Serie' &&
                                    is_array($genres) &&
                                    in_array('drame', $genres);
                            });
                        @endphp

                        @if ($Toutseries->isEmpty())
                            <p>Aucune série dramatique disponible pour l'instant</p>
                        @else
                            @foreach ($Toutseries as $newseries)
                                <div class="card mx-4">
                                    <div class="card__cover card_film">
                        ...
                        @php
                            $Toutseries = $data['Toutseries']->filter(function ($newseries) {
                                ...
                                return $newseries->status == 1 &&
                                    $newseries->type == 'Serie' &&
                                    is_array($genres) &&
                                    in_array('comedie', $genres);
                            });
                        @endphp

                        @if ($Toutseries->isEmpty())
                            <p>Aucune série comédie disponible pour l'instant</p>
                        @else
                            @foreach ($Toutseries as $newseries)
                                <div class="card mx-4">
                                    <div class="card__cover card_film">
                        ...
                        @php
                            $Toutseries = $data['Toutseries']->filter(function ($newseries) {
                                ...
                                return $newseries->status == 1 &&
                                    $newseries->type == 'Serie' &&
                                    is_array($genres) &&
                                    in_array('horreur', $genres);
                            });
                        @endphp

                        @if ($Toutseries->isEmpty())
                            <p>Aucune série d'horreur disponible pour l'instant</p>
                        @else
                            @foreach ($Toutseries as $newseries)
                                <div class="card mx-4">
                                    <div class="card__cover card_film">
                        ...
                        @php
                            $Toutseries = $data['Toutseries']->filter(function ($newseries) {
                                ...
                                return $newseries->status == 1 &&
                                    $newseries->type == 'Serie' &&
                                    is_array($genres) &&
                                    in_array('romance', $genres);
                            });
                        @endphp

                        @if ($Toutseries->isEmpty())
                            <p>Aucune série romantique disponible pour l'instant</p>
                        @else
                            @foreach ($Toutseries as $newseries)
                                <div class="card mx-4">
                                    <div class="card__cover card_film">
